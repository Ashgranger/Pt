        ...
        try:
            async with aiohttp.ClientSession() as s:
                async with s.ws_connect(
                    ...
                    print(f"{_G}  ⚡ WS#{ws_id} connected "
                          f"({len(tokens)} tokens){_Z}")
...
async def ws_manager(state: State, gstats: GlobalStats):
    ...
    while True:
        ...
        print(f"{_C}  WS: {len(shards)} connections × "
              f"~{WS_TOKENS_PER_CONN} tokens = "
              f"{len(all_toks)} tokens total{_Z}")
...
def _print_header():
    print(f"\n{_C}{_B}{'═'*70}{_Z}")
    print(f"{_B}   Polymarket  ACTIVE-MARKET  ARB DETECTOR  ·  µs PRECISION{_Z}")
    print(f"{_C}{'═'*70}{_Z}")
    print(f"  JSON lib    : {_Y}{_JSON_LIB}{_Z}")
    print(f"  Min spread  : {_Y}{MIN_ARB_SPREAD:.4f}{_Z} ({MIN_ARB_SPREAD*100:.2f}¢)")
    print(f"  Active only : {_Y}{ACTIVE_ONLY}{_Z}  "
          f"Vol≥{_Y}{MIN_VOLUME_24H}{_Z}  Liq≥{_Y}{MIN_LIQUIDITY}{_Z}")
    print(f"  WS shard sz : {_Y}{WS_TOKENS_PER_CONN}{_Z} tokens/connection")
    print(f"{_C}{'─'*70}{_Z}\n")
...
def _print_arb_open(ev: ArbEvent, yes: float, no: float):
    ...
    print(f"{_G}   YES={yes:.5f}  NO={no:.5f}  "
          f"SUM={yes+no:.5f}  "
          f"SPREAD={_B}{ev.max_spread:.5f}{_Z}{_G} "
          f"({ev.max_spread*100:.3f}¢){_Z}")


def _print_arb_close(ev: ArbEvent, n: int):
    print(f"\n{_R}⏹  [{_ts()}] ARB CLOSED #{n}  {'─'*36}{_Z}")
...
def _print_stats(gstats: GlobalStats,
    ...
    print(f"{_C}{'─'*70}{_Z}")
    print(f"  Markets active   : {_Y}{gstats.markets_active}{_Z}  "
          f"(total crawled: {gstats.markets_total})")
    print(f"  WS connections   : {_Y}{gstats.ws_connections}{_Z}")
    ...
    if open_arbs:
        ...
        for cid, ev in list(open_arbs.items())[:10]:
            m = markets.get(cid)
            spread = m.spread() if (m and m.ready()) else ev.max_spread
            print(f"    {_G}●{_Z} {ev.question:<52}  "
                  f"Δ={_G}{spread:.5f}{_Z}  "
                  f"t={_G}{_fns(ev.duration_ns())}{_Z}")

    # Leaderboard — top by max spread (all time)
    if gstats.history:
        top = sorted(gstats.history,
                     key=lambda e: e.max_spread, reverse=True)[:TOP_ARB_COUNT]
        print(f"\n{_Y}  LEADERBOARD — TOP {TOP_ARB_COUNT} BY MAX SPREAD{_Z}")
        print(f"  {'#':>3}  {'Dur':>12}  {'MaxΔ':>8}  {'AvgΔ':>8}  "
              f"{'Smpl':>5}  Question")
        print(f"  {'─'*3}  {'─'*12}  {'─'*8}  {'─'*8}  "
              f"{'─'*5}  {'─'*38}")
        for i, ev in enumerate(top, 1):
            print(f"  {i:>3}  {_fns(ev.duration_ns()):>12}  "
    ...
    # Recent closed
    if gstats.history:
        recent = list(gstats.history)[-6:]
        print(f"\n{_C}  RECENT CLOSED ARBS{_Z}")
        for i, ev in enumerate(recent,
                               max(1, gstats.total_events - len(recent) + 1)):
            print(f"  #{i}  {_fns(ev.duration_ns()):>12}  "
                  f"maxΔ={ev.max_spread:.5f}  {ev.question[:42]}")
