...
MIN_ARB_SPREAD      = 0.000    # 0¢ — catch ANY positive spread (yes+no < 1.0)
MIN_YES_ASK         = 0.02     # YES ask must be > 2¢  
MIN_NO_ASK          = 0.02     # NO  ask must be > 2¢
SHOW_TOP_N          = 10       # top-N spreads in stats (closest to arb)
MIN_VOLUME_24H      = 1.0      # min $1 traded in last 24h
MIN_LIQUIDITY       = 0.0      # min $ liquidity
REQUIRE_ACCEPTING   = True     # only acceptingOrders:true
ACTIVE_ONLY         = True     # only active=true markets
...
def _print_stats(gstats: GlobalStats,
    ...
    # Count how many markets have both prices set
    ready = sum(1 for m in markets.values() if m.ready())
    positive = sum(1 for m in markets.values()
                   if m.ready() and m.yes_ask >= MIN_YES_ASK
                   and m.no_ask >= MIN_NO_ASK and m.spread() > 0)

    print(f"\n{_C}── STATS {_ts()} ──{_Z}")
    print(f"  markets={gstats.markets_active}  priced={ready}  "
          f"YES/NO≥{MIN_YES_ASK:.2f} & spread>0: {_G}{positive}{_Z}  "
          f"arbs_total={gstats.total_events}  "
          f"avg={avg_ms:.1f}ms  max-Δ={gstats.max_spread:.4f}")
    print(f"  ticks={total_ticks} ({ws_pct}%WS)  "
          f"WS-conn={gstats.ws_connections}  err={gstats.errors}")

    # Show spread distribution — key diagnostic
    if top_spreads:
        print(f"\n{_Y}  TOP {SHOW_TOP_N} by spread  "
              f"(arb = spread > 0, i.e. YES_ask + NO_ask < 1.0){_Z}")
        print(f"  {'Spread':>9}  {'YES_ask':>8}  {'NO_ask':>8}  "
              f"{'YES+NO':>8}  Question")
        print(f"  {'─'*9}  {'─'*8}  {'─'*8}  {'─'*8}  {'─'*40}")
        for spread, m, ya, na in top_spreads:
            if m is None: continue
            arb_tag = f" {_G}{_B}◄ ARB!{_Z}" if spread > 0 else ""
            color = _G if spread > 0 else (_Y if spread > -0.005 else "")
            print(f"  {color}{spread:>+9.5f}  {ya:>8.5f}  {na:>8.5f}  "
                  f"{ya+na:>8.5f}  {m.question[:40]}{_Z}{arb_tag}")

    if open_arbs:
        print(f"\n{_G}  LIVE ARBS ({len(open_arbs)}):{_Z}")
        for cid, ev in list(open_arbs.items())[:5]:
            m = markets.get(cid)
            s = m.spread() if (m and m.ready()) else ev.max_spread
            print(f"  {_G}● {ev.question[:52]}  "
                  f"Δ={s:.5f}  t={ev.duration_ns()//1_000_000}ms{_Z}")

    if gstats.history:
        print(f"\n{_C}  HISTORY (last {min(10, len(gstats.history))}){_Z}")
        for ev in list(gstats.history)[-10:]:
            print(f"    dur={ev.duration_ns()//1_000_000}ms  "
                  f"maxΔ={ev.max_spread:.5f}  {ev.question[:50]}")
    print()
...
async def detector(state: State, gstats: GlobalStats):
    ...
    while True:
        ...
        for cid, m in markets.items():
            ...
            is_arb = spread > MIN_ARB_SPREAD  # spread > 0 means YES+NO < 1.0
