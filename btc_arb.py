...
import asyncio, json, os, time, concurrent.futures, hmac, hashlib, base64
...
_hb_id      = ""
_clob       = None
_http: aiohttp.ClientSession = None   # shared session for order POSTs
_executor   = concurrent.futures.ThreadPoolExecutor(max_workers=4, thread_name_prefix="clob")

# ── Order signing (CPU-only, fast) ────────────────────────────────────────
# Sign in thread (uses eth crypto), then POST via aiohttp (true async HTTP).
# This cuts placement from ~2000ms → ~50-150ms per leg, both fire in parallel.

def _sign_order(token_id, price, size, tick_size, neg_risk):
    """Pure CPU: build + sign the order struct. Returns signed order dict."""
    try:
        signed = _clob.create_order(
            OrderArgs(
                token_id = token_id,
                price    = round(price, 4),
                size     = round(size, 2),
...
async def _place(token_id, price, size, tick_size, neg_risk):
    """
    1. Sign order in thread pool (CPU, ~5ms)
    2. POST signed order via aiohttp (async HTTP, ~50ms)
    Both legs run truly in parallel via asyncio.gather.
    """
    loop = asyncio.get_running_loop()

    # Step 1: sign (blocking CPU call → thread)
    signed = await loop.run_in_executor(
        _executor, _sign_order, token_id, price, size, tick_size, neg_risk
    )
    if signed is None:
        return {"errorMsg": "signing failed"}

    # Step 2: build L2 HMAC auth headers
    # Try SDK method first; fall back to manual HMAC-SHA256 if not available.
    try:
        headers = _clob.get_order_headers()
    except Exception:
        # Manual L2 HMAC signing (same as SDK internals)
        ts_now  = str(int(time.time()))
        message = ts_now + "POST" + "/order"
        ...
        headers = {
            "POLY_ADDRESS":    os.getenv("FUNDER_ADDRESS", ""),
            "POLY_SIGNATURE":  sig,
            "POLY_TIMESTAMP":  ts_now,
            "POLY_API_KEY":    os.getenv("API_KEY", ""),
            "POLY_PASSPHRASE": os.getenv("API_PASSPHRASE", ""),
        }

    # Step 3: POST directly — no SDK HTTP overhead
    payload = {
        "order":     signed,
        "orderType": "GTC",
    ...
    try:
        async with _http.post(
            ...
            return data if isinstance(data, dict) else {"errorMsg": str(data)}
    except Exception as e:
        return {"errorMsg": str(e)}

# ── Cancel / poll / heartbeat (still via SDK in thread — not latency critical) ──
...
async def main():
    ...
    # Shared high-perf session: TCP keepalive + connection pool
    _conn = aiohttp.TCPConnector(limit=20, ttl_dns_cache=300, enable_cleanup_closed=True)
    _order_session = aiohttp.ClientSession(
        connector = _conn,
        headers   = {
            "User-Agent":   "btc-arb/3.0",
            "Content-Type": "application/json",
            "Accept":       "application/json",
        },
    )
    global _http
    _http = _order_session
